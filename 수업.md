...
## multiplexing
##### server에는 여러개의 소켓이 있고 client A, B 와 통신하고 있다고 하자.
##### server가 data 를 send 할 때 각 socket에서 오는 데이터를 받아서 transport header를 붙여 segment로 만들고 segment를 network layer에 전달한다.
## demultiplexing
##### segment를 header의 destination port를 확인, 각 data 들이 올바른 socket에 전달 될 수 있게 한다
